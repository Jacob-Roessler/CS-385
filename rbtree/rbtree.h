...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // throw tree exception if duplicate is found
        if(find(key).node_ptr){
        	std::stringstream ss;
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // return if no node remain
    	if(n == nullptr){
    		return;
    	}
    	//must reference left and right before deleting n
    	delete_tree(n->left);
    	delete_tree(n->right);
    	//delete n
    	delete n;
    ...
    void insert_fixup(Node<K, V> *z) {
        // create pointer on stack
    	Node<K, V> *y;
    	//make sure z is not root because nullptr is not a node so it has no parent
    ...
    void right_rotate(Node<K, V> *x) {
        //code in textbook rights replaced with lefts and vice versa
        Node<K, V> *y = x->left;
        x->left = y->right;
        if(y->right != nullptr)
            y->right->parent = x;
    ...
    int height(Node<K, V> *node) const {
    	...
    	//return the height of the side with the most depth
    ...
    size_t leaf_count(Node<K, V> *node) const {
    	...
    	//leaf case
    		...
    		return 1 + leaf_count(node->left) + leaf_count(node->right);
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
    	...
    	//if a node has a left or right child
    	if(node->left || node->right)
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	// return 3 + because its (current + left + right) + (heightL + heightR)
