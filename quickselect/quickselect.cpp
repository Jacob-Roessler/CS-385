...
int main(int argc, char *argv[]) {
    ...
    if(k-1 >= num_values){
        cerr << "Error: Cannot find smallest element " << k <<  " with only " << values.size() <<" value" << (values.size() == 1 ? "." : "s.") << endl;
        return 1;
    }
    // TODO - call the quick_select function and display the result
    cout << "Smallest element " << k << ": "<< quick_select(values.data(), values.size(), k);
